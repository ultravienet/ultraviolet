...
pub fn prepare_issue(
    ...
    let asset = uv_air::wots::public_key(&uv_air::wots::permutation(), &keys.wots_seed);
...
pub struct SendPlan {
    ...
    /// original paid.
    pub replays: Vec<uv_air::wots::Digest>,
    /// Fresh parts: which note funds it, how much, and the payee slot it pays.
    pub fresh: Vec<(uv_air::wots::Digest, u64, crate::address::Slot)>,
...
pub fn plan_send(
    ...
    amount: u64,
    from: Option<&uv_air::wots::Digest>,
    ...
    // (spec/99 [MERGE]).
    let plan: Vec<(uv_air::wots::Digest, Amount)> = match from {
    ...
    let replays: Vec<uv_air::wots::Digest> = plan
        .iter()
        .filter(|(c, _)| {
            wallet
                .store
        ...
        .map(|(c, _)| *c)
        .collect();
    let fresh_parts: Vec<(uv_air::wots::Digest, Amount)> = plan
        .iter()
...
/// in.
fn decode_digest(s: &str) -> Option<uv_air::wots::Digest> {
