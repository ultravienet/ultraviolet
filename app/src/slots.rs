...
//!
//! **The most dangerous small file in the system.** Each slot on an address
//! carries one WOTS+ one-time key. Build two notes on one slot, sign both, and
//! the key is disclosed — not a lost payment, a lost key. Nothing on the
//! payee's side can stop this, because which slots a payer has consumed is
...
/// [`tests::the_reservation_filename_is_stable_forever`].
        ...
        if used.contains(i) {
            return Err(Error::Refused(format!(
                "slot {i} is already spent for this address; reusing it would put two \
                 notes under one one-time key"
            )));
        }
    }
    used.extend_from_slice(newly);
    let p = reservation_path(home, addr_id);
