...
//! gets its own nullifier key, its own commitment randomness, and its own asset
//! binding, all derived from `(wallet seed, note index)` with distinct sponge
//! domains. Consequences, each load-bearing:
//!
//! - **One-time secrets are per note by construction.** A note is spent once,
//!   and the nullifier key that authorizes it is witness to exactly that spend.
//!   (The wallet still owes the sign-log replay discipline for the in-flight
//!   window — `SPEC.md` §6.)
...
//!   in-flight transfers needs separate persistence (`SPEC.md` §6).
//!
...
pub struct NoteKeys {
    ...
    pub anchor: Digest,
    /// This note's asset identifier, used when the note is an **issuance**.
    ///
    /// Unforgeable because deriving it needs the wallet seed, and never a
    /// signature — spend authorization is the anchor preimage (SPEC.md §8.4).
    /// Until 2026-07-30 this field was a signing seed whose *public key* was the
    /// asset id, which meant computing one identifier walked 131 hash chains.
    /// The signature scheme it belonged to had already been deleted.
    pub asset_id: Digest,
...
/// one). This is the one convention for turning 32 seed bytes into field
/// elements, used everywhere that needs it.
...
pub fn derive(seed: &WalletSeed, index: u64) -> NoteKeys {
    ...
    NoteKeys {
        asset_id: hash(Domain::AssetId, &input),
...
mod tests {
    ...
    fn notes_get_independent_keys() {
        ...
        assert_ne!(a.nullifier_key, a.asset_id);
    ...
    fn derivation_is_deterministic_and_seed_sensitive() {
        ...
        assert_eq!(derive(&seed, 7).asset_id, derive(&seed, 7).asset_id);
        let other = WalletSeed([11u8; 32]);
        assert_ne!(derive(&seed, 7).asset_id, derive(&other, 7).asset_id);
    }
