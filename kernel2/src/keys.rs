...
pub struct NoteKeys {
    ...
    pub anchor: Digest,
    /// Seed for a one-time keypair. **Nothing on the money path signs with
    /// it** (spend authorization is the anchor preimage, SPEC.md §8.4); its
    /// one live use is the issuance note, whose derived public key *is* the
    /// asset id ([`uv_air::wots::public_key`]) — an unforgeable identifier,
    /// never a signature.
    pub wots_seed: [u8; 32],
