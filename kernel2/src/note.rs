...
//!
//! `owner_pk` is the note's one-time WOTS+ public key
//! ([`uv_air::wots::public_key`] of the per-note seed).
//!
...
/// Sponge input length of a commitment: asset + 4 amount limbs + owner_pk +
/// nullifier_key + randomness.
const COMMIT_INPUT: usize = 8 + 4 + 8 + 8 + 8;
...
pub struct Note {
    ...
    pub asset: Digest,
    pub amount: Amount,
    /// One-time WOTS+ public key authorized to spend this note.
    pub owner_pk: Digest,
    /// The spend anchor `t = H(nullifier_key)` ([`crate::keys::anchor_of`]).
    /// Public: safe to publish in an address and hand to a payer.
...
impl Note {
    /// Build a note owned by `keys`, deriving the WOTS+ public key.
    pub fn build(asset: Digest, amount: Amount, keys: &NoteKeys) -> Note {
        let perm = uv_air::wots::permutation();
        Note {
            asset,
            amount,
            owner_pk: uv_air::wots::public_key(&perm, &keys.wots_seed),
    ...
    pub fn commitment(&self) -> Digest {
        ...
        input.extend_from_slice(&self.owner_pk);
...
mod tests {
    ...
    fn every_field_binds_the_commitment() {
        ...
        let mut n = note.clone();
        n.owner_pk[0] += BabyBear::ONE;
        assert_ne!(n.commitment(), base, "owner must bind");
