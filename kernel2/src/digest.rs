...
/// Machine-checked proofs over the production codec (`cargo kani -p
/// uv-kernel2`). The doc above claims both compositions are identities on
/// their valid domains; the tests below pin that for sample values, and these
/// prove it for **every** value — which matters because anything keyed by
/// digest bytes (records on Bitcoin above all) needs exactly one encoding per
/// value, with no counterexample rather than none found.
#[cfg(kani)]
mod proofs {
    use super::*;

    /// ∀ canonical digests d — decode(encode(d)) == Some(d).
    #[kani::proof]
    fn every_digest_round_trips() {
        ...
        let d: Digest = core::array::from_fn(|i| BabyBear::from_u32(raw[i]));
        assert_eq!(decode(&encode(&d)), Some(d));
    }

    /// ∀ byte strings b — if decode accepts b, then encode gives back exactly
    /// b. So every digest has ONE byte representation: no aliases, no
    /// malleability, nothing for two chain records to disagree about.
    #[kani::proof]
    fn accepted_bytes_are_the_canonical_encoding() {
        let bytes: [u8; DIGEST_BYTES] = kani::any();
