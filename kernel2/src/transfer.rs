//! Transfers and the bundle hash — what a proof is checked against.
//!
...
//! commitment, and the **history digest the spender starts from**. It enters
//! the proof's Fiat-Shamir transcript as a public value, so everything a
//! receiver must not let the sender re-negotiate has to be inside it:
//!
//! - outputs: or the proof could be re-aimed at different payees;
...
impl Transfer {
    ...
    /// The bundle hash: what the record commits to, and a public value of the
    /// proof.
    ///
    /// Delegates to `uv_air::prove::bundle_hash`, which is the single
    /// definition — the statement a proof is checked against derives it the
    /// same way, so the proved thing and the validated thing cannot drift
    /// apart.
