...
/// Machine-checked proofs over the *production* functions above — not a model
/// of them. Run with `cargo kani -p uv-kernel2`; ordinary builds and tests
/// never compile this module.
///
/// Why these three properties: this file is the seam where the host's `u64`
/// and the circuit's four range-checked limbs must mean the same number. The
/// tests above check a handful of values; a bounded model checker proves the
/// claims for **every** `u64` / every limb quadruple, which is the difference
/// between "we tried some" and "there is no counterexample". The free-mint
/// bug this project actually shipped was a host-side comparison subtly weaker
/// than the model said — exactly the class these harnesses close.
#[cfg(kani)]
mod proofs {
    use super::*;

    /// ∀ v: u64 — the limb encoding round-trips exactly.
    #[kani::proof]
    fn every_u64_round_trips_through_limbs() {
        let v: u64 = kani::any();
        let a = Amount(v);
        assert_eq!(Amount::from_limbs(&a.limbs()), Some(a));
    }

    /// ∀ limb quadruples — `from_limbs` accepts iff every limb is 16-bit, so
    /// no aliased (out-of-range) representation of any value is ever taken.
    /// Together with the round trip this makes the representation bijective
    /// on the accepted domain: one value, one representation.
    #[kani::proof]
    fn from_limbs_accepts_exactly_the_canonical_range() {
        let raw: [u32; LIMBS] = kani::any();
        // BabyBear canonical values are < p; from_u32 asserts that, so
        // constrain the nondet input to what a field element can hold.
        for r in &raw {
            kani::assume(*r < 0x7800_0001); // BabyBear p = 15·2^27 + 1
        }
        let limbs: [BabyBear; LIMBS] = core::array::from_fn(|i| BabyBear::from_u32(raw[i]));
        let out = Amount::from_limbs(&limbs);
        let in_range = raw.iter().all(|r| *r < (1 << LIMB_BITS));
        assert_eq!(out.is_some(), in_range);
        if let Some(a) = out {
            // ...and what it decodes to is the little-endian value of those
            // limbs, so acceptance never reinterprets.
            let expect = raw
                .iter()
                .enumerate()
                .fold(0u64, |acc, (i, r)| acc | (u64::from(*r) << (LIMB_BITS * i)));
            assert_eq!(a.0, expect);
        }
    }

    /// ∀ a, b — checked addition is `None` exactly on overflow and exact
    /// otherwise; the wallet's conservation arithmetic can never wrap.
    #[kani::proof]
    fn checked_add_is_exact_or_refuses() {
        let a: u64 = kani::any();
        let b: u64 = kani::any();
        match Amount(a).checked_add(Amount(b)) {
            Some(sum) => {
    ...
    /// ∀ inputs (three of them, the transfer shape: one in, two out) —
    /// `checked_sum` is the exact sum or `None` on overflow, never a wrap.
    #[kani::proof]
    fn checked_sum_of_a_transfer_shape_never_wraps() {
        let xs: [u64; 3] = kani::any();
        let amounts = [Amount(xs[0]), Amount(xs[1]), Amount(xs[2])];
        match checked_sum(&amounts) {
            Some(total) => {
                let exact = xs[0].checked_add(xs[1]).and_then(|s| s.checked_add(xs[2]));
                assert_eq!(Some(total.0), exact);
            }
            None => {
                assert!(xs[0]
                    .checked_add(xs[1])
                    .and_then(|s| s.checked_add(xs[2]))
                    .is_none());
            }
        }
    }
...
mod tests {
    ...
    /// on the money path. the transfer circuit claimed a test pinned this; none
    /// did, until an audit noticed. This is that test.
    #[test]
    fn the_circuit_agrees_on_how_many_limbs_an_amount_has() {
        assert_eq!(LIMBS, uv_air::sponge::AMOUNT_LIMBS);
