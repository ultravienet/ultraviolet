...
mod hop_test {
    ...
    fn a_full_hop_holds_together() {
        ...
        // The bundle hash is the one message note 0's key ever signs.
        let msg = transfer.bundle_hash();
        let sig = wots::sign(&perm, &alice_keys.wots_seed, &msg);
        assert!(
            wots::verify(&perm, &note.owner_pk, &msg, &sig),
            "the note's own key must authorize its spend"
        );

        // A different transfer is a different message — the signature does not
        // transfer to a substituted payee.
        let mut hijacked = transfer.clone();
        hijacked.outputs[0] =
            Note::build(asset, Amount(60), &derive(&WalletSeed([66u8; 32]), 0)).commitment();
        assert!(!wots::verify(
            &perm,
            &note.owner_pk,
            &hijacked.bundle_hash(),
            &sig
        ));
