...
//! from `h`, so the ancestry itself must travel with the note — the O(n)
//! receive cost `SPEC.md` §8 states as a limit. The accumulator design (spec/99 `[ACC]`) would change that;
