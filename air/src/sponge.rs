...
//! permutation is the same published-constants Poseidon2-16 the STARK uses
//! ([`crate::poseidon2::permutation`]), so a hash computed here is exactly a
//! hash the circuit can constrain as one row.
//!
//! ## Construction
//!
//! Rate 8, capacity 8, add-absorb:
//!
//! ```text
...
use crate::poseidon2::{self, Digest, N, WIDTH};
...
pub enum Domain {
    ...
    /// Per-note owner-secret derivation (kernel2 `keys`).
    ...
    /// An asset's identifier, derived from the issuing note's keys.
    ///
    /// **Added 2026-07-30**, and it invalidates every asset id minted before
    /// that date — the construction changed, deliberately, while every asset in
    /// existence is play money. The journal records what it replaced.
    AssetId = 0x_4153_5354, // "ASST"
}
