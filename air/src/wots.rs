//! WOTS+ one-time signatures over Poseidon2 — **off the money path since
//! 2026-07-29**, kept for what still needs it.
//!
//! ## Why this module survives the proof-auth migration
//!
//! No spend signs or verifies anything: authorization is the in-circuit
//! anchor-preimage (SPEC.md §8.4), and the signature circuit that once
//! verified these chains is deleted. Three things keep this file:
//!
//! 1. **The shared types.** [`Digest`], [`N`], [`WIDTH`], and
//!    [`permutation`] are the field-element plumbing every crate imports.
//!    They are not signature machinery; they merely live here historically.
//! 2. **The asset id, which is live.** An asset's id *is* the issuance
//!    note's one-time public key ([`public_key`] over the issuer's seed):
//!    public by nature, deterministic, and unforgeable without the seed —
//!    used as an identifier, never for signing
//!    (`uv-app::commands::prepare_issue`; pinned by
//!    `wallet2/tests/an_anchor_does_not_reveal_the_genesis_nullifier.rs`).
//! 3. **The planned mint authority.** Reissuance (spec/99 `[SUPPLY]`) is
//!    designed as a hash-chain of one-time keys, each naming its successor —
//!    attestation only, never spend validity. [`sign`]/[`verify`] have no
//!    production caller today and exist for that; the repository keeps one
//!    always-amended commit, so deleting them is unrecoverable rather than
//!    revertible.
//!
//! ## The historical argument, kept honest
//!
//! This section used to argue for WOTS+ *on the money path*: leak-tolerance,
//! because SP1's raw STARK was not zero-knowledge and a bare preimage in an
//! unblinded witness is a bearer secret. Both premises died — the zkVM was
//! deleted, and the hand-written circuit's **hiding configuration is the
//! payment format**, which is exactly what makes the bare anchor-preimage
//! design safe to ship. The measured verdict that removed the signature
//! (6.7× slower than proof-native authorization) is in SPEC.md §8.4.
//!
//! ## The one-time rule, where it still applies
//!
//! Signing twice with one key **reveals the private key** — WOTS+ leaks a
//! chain element per signature. On the money path this hazard is gone with
//! the signatures; for the future mint chain it is the design constraint:
//! each link signs exactly once, naming its successor.
//!
//! ## Parameters
//!
//! Classic WOTS+ at `w = 16` over a 32-byte digest: 64 message digits, 3
//! checksum digits, 67 chains of ≤15 steps. `LOG_W` is a constant so the
//! size/steps tradeoff can be measured rather than argued.
