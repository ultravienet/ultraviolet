...
//!   `Poseidon2Cols`, so a wider row (ours carries the transfer's own columns
...
//! honest witnesses.
//!
//! The differential tests in `tests/poseidon2_differential.rs` are therefore necessary and
//! **not sufficient**: they establish that the circuit agrees with
//! [`crate::poseidon2`] on the cases exercised, not that no *other* witness satisfies
...
// They live here, beside the evaluation they parameterize.
...
/// the constrained permutation output — which is what [`crate::authproto_air`]
/// chains into the next row's `inputs`.
///
