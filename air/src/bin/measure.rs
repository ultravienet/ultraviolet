...
//!
//! Since the WOTS+ signature was removed from the money path (spec/99
//! `[PROOF-AUTH]`, 2026-07-29), there is one circuit to measure — the
//! anchor-preimage transfer (`authproto_air`). `UV_MEASURE=standard|hiding` runs
...
fn main() {
    ...
    println!(
        "\nThe hiding config is the payment format. vs the zkVM it replaced:\n\
         SP1 124 s / 1242 KB / 9.67 GB peak per hop."
    );
}
