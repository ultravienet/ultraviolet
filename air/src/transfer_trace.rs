...
///
/// Authorization is the anchor preimage (the spender supplies `input.nullifier_key`),
/// not a signature — the WOTS+ `sig` field was removed with the signature circuit
/// (spec/99 `[PROOF-AUTH]`). `msg` is the bundle hash, public, bound to the proof
/// through Fiat–Shamir over the public values.
