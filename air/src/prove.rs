...
//! against the gate, so spending it bought the smaller proof (spec/04).
//!
...
//!   since the WOTS+ signature was removed (spec/99 `[PROOF-AUTH]`), so hiding is
//!   not optional on the money path.
...
// ---------------------------------------------------------------------------
// The proof-native authorization PROTOTYPE (`crate::authproto_air`).
//
// Measurement-grade, deliberately NOT consensus: nothing in the wallet calls
// these, `air/mutants.py` does not sweep the prototype's constraints, and no
// money moves on them. They exist so the WOTS+ decision rule (spec/99
// [PROOF-AUTH]) is applied to a measured number instead of a projection.
// ---------------------------------------------------------------------------

/// A prototype proof. No tips: there is no signature for tips to belong to,
