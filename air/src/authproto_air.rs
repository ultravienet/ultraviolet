...
//!
//! This is *the* money-path circuit, and the only one (spec/99 `[PROOF-AUTH]`,
//! since 2026-07-29). It began as a measurement prototype and keeps the name it
//! was given then.
//!
//! ## What it proves
//!
...
//! transcript. There is no signature beside it and nothing else to check: the
//! trace is 16 rows, one Poseidon2 permutation each.
//!
//! That is also the system's sharpest operational hazard, removed: **proving
//! twice reveals nothing.** There is no key that a second use discloses, so the
//! sign-log, replay-instead-of-re-prepare, and slot reservations defend
//! idempotence and linkability rather than a secret.
//!
...
impl<AB: AirBuilderWithPublicValues<F = BabyBear>> Air<AB> for AuthProtoAir {
    fn eval(&self, builder: &mut AB) {
        ...
        // 4b. Padding rows carry no witness either — the same rule as 4, for the
        //     columns 4 does not reach.
        //
        //     **Found 2026-07-30 by the pairwise probe, and it is 4's own
        //     argument left half-applied.** Constraint 4 says a padding row's
        //     permutation lanes must be zero because they "would otherwise be 16
        //     free lanes per row". Exactly the same was true of the witness
        //     columns one block later, and nothing said so: on the single padding
        //     row, `NK`, `T` and the three amount fields were **28 field elements
        //     a prover could choose freely**, read by no constraint.
        //
        //     Not a soundness hole on its own — a cell nothing reads cannot make
        //     an invalid statement verify — but it is precisely the shape of
        //     `[EXPORT-COL]`, which this project already found and *closed*
        //     ("a column nothing reads is a column a prover controls", constraint
        //     2 above). Leaving 28 of them would have made that closure false.
        //     "Read by nothing" is a property of today's constraints, not of the
        //     circuit, and it is the kind of slack a future rule quietly starts
        //     depending on.
        //
        //     The honest trace already writes zeros here — `generate` skips
        //     padding rows entirely — so this costs nothing and rejects nothing
        //     that was previously accepted.
        for c in NK..AMT_O1 + AMOUNT_LIMBS {
            builder.assert_zero((AB::Expr::ONE - is_sponge.clone()) * local[c].clone().into());
        }
...
mod proving_tests {
    ...
    fn commitment(asset: &poseidon2::Digest, o: &NoteOpening) -> poseidon2::Digest {
        let mut p = Vec::new();
