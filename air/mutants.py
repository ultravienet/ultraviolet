...
FILES = [
    # The one money-path circuit. The WOTS+ signature circuit (`wots_air.rs`) and
    # the WOTS+ transfer circuit (`transfer_air.rs`) were deleted 2026-07-29 when
    # the money path cut over to proof-native authorization (spec/99
    # `[PROOF-AUTH]`), so this is the only source the sweep now touches.
...
def main():
    ...
    # ever true for Ctrl-C. `wots_air.rs` was left with constraint 9 commented
    # out — a real soundness hole sitting in the working tree of a repository
