...
enum Cmd {
    ...
    /// Catch a local index up from a mirror, then answer a lookup from it.
    ///
    /// This is what a phone does: pull **pages of everything** and question
    /// the result in-process. No nullifier is sent anywhere — the mirror is
    /// never told which coin is being asked about, and offers no endpoint
    /// that would accept one.
    MirrorSync {
        /// Base URL of a mirror, e.g. `http://127.0.0.1:8788`.
        #[arg(long = "from")]
        url: String,
        /// Where to keep the synced index.
        #[arg(long, default_value = "./uv-mirror-index.json")]
        index: PathBuf,
        /// A nullifier (hex) to look up **locally** after syncing, to show the
        /// answer comes from this device. Nothing about it is ever sent.
        #[arg(long)]
        probe: Option<String>,
    },
}
...
fn main() {
    ...
    match cli.cmd {
        ...
        Cmd::MirrorSync { url, index, probe } => cmd_mirror_sync(&url, &index, probe.as_deref()),
...
/// Sync from a mirror and report what the view can now answer.
fn cmd_mirror_sync(url: &str, index: &Path, probe: Option<&str>) {
    use uv_wallet2::chain::Chain;

    let view = uv_btc::mirror::MirrorView::open(index);
    let before = view.next_height();
    let synced = uv_btc::mirror_fetch::sync_all(&view, url).unwrap_or_else(|e| {
        eprintln!("mirror sync failed: {e}");
        eprintln!("the view is unchanged and still refuses lookups, which is the safe answer");
        std::process::exit(1);
    });
    println!(
        "synced {} -> {} (mirror tip {}), {} page(s)",
        before,
        synced.covered_through,
        synced.tip,
        synced.digests.len()
    );
    if !synced.complete() {
        println!("NOT caught up — every lookup answers `unanswerable` until it is.");
        println!("That refusal is the point: a view behind the tip cannot tell");
        println!("\"no record exists\" from \"I have not looked there yet\".");
        std::process::exit(1);
    }
    println!("caught up; this view answers its own lookups locally");

    // Prove it: ask about a nullifier the mirror was never told. A miss is a
    // real answer only because the view is complete; a hit proves the record
    // travelled in the bulk pages and was found without a query.
    let nf = match probe {
        Some(hexs) => try_unhexd(hexs).unwrap_or_else(|| {
            eprintln!("--probe is not a canonical digest");
            std::process::exit(1);
        }),
        None => [p3_baby_bear::BabyBear::new(0xDEAD); 8],
    };
    match view.first_occurrence(&nf) {
        uv_wallet2::chain::Lookup::None => {
            println!("probe lookup answered locally: NO record for that nullifier");
            if probe.is_some() {
                eprintln!("...but a nullifier was named, so it should have been found");
                std::process::exit(1);
            }
        }
        uv_wallet2::chain::Lookup::Found(o) => println!(
            "probe lookup answered locally: FOUND at depth {} — the record arrived in a bulk page",
            o.depth
        ),
        uv_wallet2::chain::Lookup::Unanswerable => {
            eprintln!("view reports unanswerable despite being caught up — a bug");
            std::process::exit(1);
        }
    }
    println!("issuances visible: {}", view.issuances().len());
}
