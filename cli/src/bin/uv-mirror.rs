...
fn main() {
    ...
    // Loaded with floor 0: a mirror that cannot answer for the whole chain is
    // a mirror whose "nothing found" is not an answer. `load` discards an
    // index built from a later floor rather than pretending, so a mirror
    // started against a partial index serves an empty view and says so below.
    let index = Arc::new(Mutex::new(RecordIndex::load(&index_path, 0)));
...
fn serve(mut stream: TcpStream, index: &Mutex<RecordIndex>, path: &std::path::Path) {
    ...
    let fresh = RecordIndex::load(path, 0);
