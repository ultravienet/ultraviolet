...
impl Transport for SignalCli {
    ...
    fn take(&self, inbox: &Path) -> Result<usize, String> {
        ...
        let mut written = 0usize;
        // A freshly linked device has no attachments directory at all —
        // signal-cli creates it when something with an attachment first
        // arrives. That is "no mail yet", not a failure, and reporting it as an
        // error made a working setup look broken on its very first scan.
        let entries = match std::fs::read_dir(&self.attachments) {
            Ok(e) => e,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(0),
            Err(e) => return Err(format!("cannot read {}: {e}", self.attachments.display())),
        };
