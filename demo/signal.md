...
claim in [spec/05](../spec/05-NETWORK.md).
...
own double spend ([spec/03](../spec/03-RECORDS.md)).
...
You can verify the mechanics with one account by paying yourself:

```bash
export UV_SIGNAL_TO="+<your own number>"      # Note to Self
```

Two homes, one Signal account, the bundle making a real round trip through
Signal's servers. It proves delivery, not a two-party payment.
...
| Date | Commit | signal-cli | What moved | Ran by |
|---|---|---|---|---|
| | | | | |
