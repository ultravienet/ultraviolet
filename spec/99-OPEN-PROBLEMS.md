...
(A fourth item, "`demo/regtest.sh` is manual", is closed: it runs in CI against a pinned
...
The ancestry rule ([03](../SPEC.md#7-records-and-settlement)) is correct but linear: O(n) chain lookups per receive,
...
- **Fresh isolating tests and a fresh `air/mutants.py` sweep**: 16 mutants, 7 killed, 9 survivors
  (`air/COVERAGE.md`, subject `c21926e59faa3ead`) — the identical kill pattern before and after
  the `owner_pk` drop, which is what "the rewrite is the same circuit" looks like from the sweep's
  side. The survivors are the sponge-lane cluster inheriting soundness from the deleted circuit's
  byte-identical, swept-clean constraints; porting the structural probes is the open work, named
  there.
...
faster and ~23× less memory than the signature circuit on the payment format. The iPhone numbers
were measured on the deleted circuit and are withdrawn until the signed app re-measures them.

**What remains open here:** the sponge-lane survivors' structural probes (above), and
`[HIDING-UNVERIFIABLE]` — zero-knowledge is now load-bearing for funds, and the property is
still not independently verifiable, which is exactly why it is named for the professional
review.
...
gate reverts to the amount-only check that shipped. This is the ITF-replay pattern the rest of the
matrix's model-only rows will follow — the model's frozen testimony is the test vector, so a code
change that diverges from the model fails a test derived from the model. `formal/regen-traces.sh`
...
cooperative-batch contents ([03](../SPEC.md#7-records-and-settlement)), and portable receipts
Needs a content-addressed blob-host interface, a replication
...
## Design (not blocking v1)
  ...
  interop profile.
- **Cooperative-batch availability `[BATCH-AVAIL]`** ([03](../SPEC.md#7-records-and-settlement)) — retention duty
...
## Watch items (external clocks)
  ...
  ([06](../SPEC.md#2-system-overview)); ours unchanged, plumbing would need rework.
- **Bitcoin's own PQ migration `[WATCH-BTC-PQ]`** (BIP-360/361) — a systemic dependency on
  ordering availability ([01](../SPEC.md#5-cryptographic-foundations)); nothing here changes when it lands.
...
- **Signal's appetite `[WATCH-SIGNAL]`** — whether the payment message type is something
  ...
  your own server forever* ([05](../SPEC.md#10-delivery)). A business question with a hard technical
...
| `[SPONGE-MARGIN]` | Sponge capacity margin | Decided rather than fixed: ~2^124 generic collision cost, accepted deliberately and written into [01](../SPEC.md#5-cryptographic-foundations) as a target instead of an accident. |
