...
## The construction (design-stage: modelled in `formal/channels.qnt`, not yet in the kernel)
...
- **Claims.** In-window, either party publishes `channel_id ‖ seq ‖ chain_element ‖ state_pointer` — the pointer resolves — via the durable blob store, which is a role the messaging carrier does not fill ([05](05-NETWORK.md)) — to the full co-signed state at that seq.
...
- **Watch duty** splits in two, and only one half is easy to delegate. *Posting your higher claim* is a Bitcoin record, so any chain-connected watchtower can do it while holding no secrets. *Keeping your latest state retrievable* needs durable storage, which the messenger does not provide ([05](05-NETWORK.md)) — so a channel participant carries a standing storage duty the base rail does not.
...
| Counterparty eclipses your storage so your latest state is unretrievable at settlement | **Theft via DA.** Mitigated by replicating your latest 16 KB state across independent blob hosts — a standing DA duty channels carry and the base rail does not. |

### What the model checked

`formal/channels.qnt` models these rules directly. Results, one module per row, safety and liveness kept separate:

| Module | Change from the spec | Theft? | Deadlock? |
|---|---|---|---|
| `disciplined` | none — the rules as written | no | no |
| `equivocating` | never-re-sign dropped | **yes** | no |
| `offline` | target unwatched past `W` | **yes** | no |
| `eclipsed` | target's storage unretrievable | **yes** | no |
| `naiveSettle` | settle at highest *claimed*, not highest *backed* | no | **yes** |

Three things follow. **The never-re-sign discipline is load-bearing** — dropping it alone, with everything else intact, produces theft, so the "mandatory" above is demonstrated rather than asserted. **The two residues are producible**, which is the standard a model of this table has to meet: had it been unable to reproduce them, the model would have been wrong, not the spec. And **"phantom claims are inert, not a freeze" holds because of the drop-to-next rule specifically**: replace it with the simpler "settle at the highest claimed seq" and an unbacked claim above the real maximum stops settlement permanently, stealing nothing. Inertness is a property of that rule, not of phantom claims.

The liveness result is a bounded viability check: whenever a backed claim exists in every state searched, settlement has a winner and is therefore enabled. Expressing this as an invariant avoids assuming a fair scheduler, but the current Apalache run is still bounded to six transitions (five for `naiveSettle`), not an inductive proof over every reachable state. It does not say settlement is *taken*: that requires a party to act within `W`, which is the assumption this tier openly makes and which `offline` prices. Two things the model does not see: in-window claim ordering is assumed stable (what the reorg margin buys), and the cryptography is assumed sound.

Two residues remain by construction — a **liveness** assumption (act within W) and a **retrievability** assumption (your latest state stays fetchable). Both are the price of resolving an absence predicate without L1 script or a bonded arbiter; both are strictly milder than Lightning's, where the same failures yield theft *plus* toxic-state and quantum-exposed custody. A bonded arbiter would dissolve both — that is exactly the optional speed layer ([11](11-SPEED-LAYER.md)), kept out of core on purpose. These rules are one of the v1 review gates — gate 3, `[CHANNELS-REVIEW]` in [99](99-OPEN-PROBLEMS.md).
