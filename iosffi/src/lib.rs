...
fn opening(perm: &Poseidon2BabyBear<16>, tag: u32, v: u64) -> NoteOpening {
...
pub extern "C" fn uv_measure(runs: u32, mode: u32) -> *mut c_char {
    ...
    if do_std {
        ...
        // Warm-up, discarded.
        let _ = prove::prove_transfer(&cfg, &witness, &publics);
        for i in 0..runs.max(1) {
            let t0 = Instant::now();
            let p = prove::prove_transfer(&cfg, &witness, &publics);
            let secs = t0.elapsed().as_secs_f64();
            let t0 = Instant::now();
            let ok = prove::verify_transfer(&cfg, &p, &publics).is_ok();
    ...
    if do_hid {
        ...
        let _ = prove::prove_transfer_hiding(&hcfg, &witness, &publics);
        for i in 0..runs.max(1) {
            let t0 = Instant::now();
            let p = prove::prove_transfer_hiding(&hcfg, &witness, &publics);
            let secs = t0.elapsed().as_secs_f64();
            let t0 = Instant::now();
            let ok = prove::verify_transfer_hiding(&hcfg, &p, &publics).is_ok();
            let vms = t0.elapsed().as_secs_f64() * 1000.0;
            out.push_str(&format!(
