...
/// The exact payload a note's key signed, plus the proof made for it.
#[derive(Clone, Serialize, Deserialize)]
pub struct SignedSpend {
    pub transfer: Transfer,
    /// Serialized `HidingTransferProof` — stored so a replay needs no
    /// re-proving (and, more importantly, no re-signing).
    pub proof: Vec<u8>,
}
