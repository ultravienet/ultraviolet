...
mod tests {
    ...
    use crate::accept::{accept, Hop, Lineage, Rejected};
    ...
    /// lineage. Its commitment is the trust anchor receivers get out of band.
    fn genesis(wallet: &mut Wallet, amount: u64) -> Note {
    ...
    fn the_full_two_hop_flow_with_all_disciplines() {
        ...
        let g = genesis(&mut issuer, 10_000);
        ...
        assert!(
            matches!(
                accept(&cfg, &chain, &asset(), &anchor, &bob_note, &lineage2, None),
        ...
        match accept(&cfg, &chain, &asset(), &anchor, &fake_note, &fab2, None) {
        ...
        let out = reconcile(&chain, &mut bob.store, &bob.log);
    ...
    fn the_inflation_attack_is_refused() {
        ...
        assert!(
            matches!(
                accept(&cfg, &chain, &asset(), &anchor, &carol_note, &lineage, None),
    ...
    fn a_frontrun_griefing_record_bricks_the_note_and_pays_the_griefer_nothing() {
        ...
        assert!(
            matches!(
                accept(&cfg, &chain, &asset(), &anchor, &bob_note, &lineage, None),
