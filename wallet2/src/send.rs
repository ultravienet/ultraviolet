...
//!
//! The recipient's note fields (owner key, nullifier key, randomness) arrive
//! from the address/KEM layer (spec/02); this module takes them as given.
...
/// layer.
/// What a payee publishes so anyone can build a note for them. All three are
/// **public**: the anchor is `H(nullifier_key)`, so handing it over does not
/// let the payer compute the nullifier this note will one day reveal.
...
pub struct Sent {
    ...
    /// input note's `wots_seed` and `nullifier_key`, handed to a caller that
...
pub fn prepare(
    ...
    // keeps every hop the same two-output shape (spec/04).
    let payment = Note {
        asset: input.asset,
        amount,
    ...
    // Log against the *signing key's* index, not the note — see `SignLog`.
    log.put(
        key_index,
        SignedSpend {
