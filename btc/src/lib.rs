...
impl Chain for BitcoinChain {
    ...
    fn issuances(&self) -> Vec<Issuance> {
        ...
        if let Ok(tip) = self.rpc.get_block_count() {
            // If the reorg check could not run, do NOT scan. See `refresh`.
            match self.detect_reorg(&mut ix, tip) {
                Ok(()) => {
                    let from = ix.next_height();
                    if from <= tip {
                        if let Err(e) = self.index_blocks(&mut ix, from, tip) {
                            eprintln!("uv: chain scan failed at height {from}..={tip}: {e}");
                        }
                        ix.save();
                    }
                }
                Err(e) => eprintln!(
                    "uv: reorg check failed at tip {tip}: {e}\n\
                     uv: NOT scanning — supply is reported from the last known-good view"
                ),
            }
        }
        ix.issuances()
    ...
    fn refresh(&self) {
        if let Ok(tip) = self.rpc.get_block_count() {
            let mut ix = self.index.lock().unwrap();
            // **Both halves of this changed on 2026-07-30, and the old version was
            // `let _ = self.detect_reorg(...)`.**
            //
            // That is the exact shape of the `index_blocks` fail-open fixed the
            // week before, on the two calls `accept` and `reconcile` make to
            // freshen their view *before judging money*. The neighbouring
            // `index_blocks` call was given error reporting; this one was not.
            //
            // Reporting alone would not have been enough. `detect_reorg` is what
            // rolls the index back when the chain it scanned is gone; if it fails
            // and we scan anyway, `next_height()` is a height on a chain that may
            // no longer exist, and every block indexed after it is appended to an
            // invalidated view. A wallet then decides whether a nullifier is spent
            // against records from an orphaned fork.
            //
            // So the failure is fail-closed: report, and **do not scan**. The view
            // stays at its last known-good height, which is a *stale* view rather
            // than a wrong one — and staleness is a state the rest of the system
            // already models and refuses on (`ViewIncomplete`), where silent
            // wrongness is not.
            match self.detect_reorg(&mut ix, tip) {
                Ok(()) => {
                    let from = ix.next_height();
                ...
                Err(e) => eprintln!(
                    "uv: reorg check failed at tip {tip}: {e}\n\
                     uv: NOT scanning — the view stays at its last known-good height \
                     rather than extending one that may have been reorganised away"
                ),
            }
        }
    }
