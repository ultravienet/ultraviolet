...
struct Entry {
    ...
    height: u64,
    index: u32,
    /// Output position within the transaction — the last component of
    /// first-occurrence order. Not `#[serde(default)]`: an index written
    /// before this field existed does not know it, and a defaulted zero would
    /// claim a position it never observed. The version bump rebuilds instead.
    vout: u32,
}

/// Bump when the on-disk shape changes. A mismatch **rebuilds** rather than
/// loading what it can — see [`RecordIndex::load`].
/// Bumped to 3: entries now carry the output index, and non-canonical records
/// are no longer admitted at all. An index built under the old rules holds
/// entries that the new rules would have rejected — so it is rebuilt rather
/// than trusted, which is exactly what this constant is for.
...
struct State {
    ...
    /// Issuance records: `(hex bytes, height)`, in confirmation order. Absent
    /// in files written before issuance existed — and `default` is right,
    /// because "this index saw no issuances" is *true* of a chain that had
    /// none. The version bump rebuilds anyway.
    #[serde(default)]
    issuances: Vec<(String, u64)>,
...
impl RecordIndex {
    ...
    /// The floor a stored index was built for, without loading it.
    ///
    /// Needed because [`load`](Self::load) *discards* an index built from a
    /// later floor than the one requested — correctly, since it cannot answer
    /// for the earlier blocks. A caller that wants to adopt whatever the file
    /// covers (a mirror serving someone else's scan) must therefore ask first
    /// rather than guess zero and silently get an empty index.
    pub fn stored_floor(path: impl AsRef<Path>) -> Option<u64> {
        let bytes = std::fs::read(path).ok()?;
        let v: serde_json::Value = serde_json::from_slice(&bytes).ok()?;
        v.get("scan_from")?.as_u64()
    }
    ...
    pub fn rollback_to(&mut self, fork_height: u64) {
        ...
        // Issuances are withdrawn by a reorg exactly like spend records. Miss
        // this and the supply total keeps counting an issuance the chain no
        // longer contains — which over-reports, the direction that matters.
        self.state.issuances.retain(|(_, h)| *h < fork_height);
    ...
    /// `vout` is the output's position within its transaction, and it is the
    /// last component of the ordering `SPEC.md` defines. Without it
    /// two records in one transaction stored identical positions and the
    /// tiebreak fell to `or_insert` plus iteration order — correct in practice,
    /// unrecorded, and impossible to audit from the stored index.
    pub fn insert(&mut self, record: &[u8; RECORD_BYTES], height: u64, index: u32, vout: u32) {
        self.state
            .first
    ...
    /// Record an issuance, in the order the chain confirmed it.
    ///
    /// A **separate list**, deliberately not the nullifier map. Those entries
    /// are keyed on the first 32 bytes of a record and governed by
    /// first-occurrence-wins; an issuance has neither property — it is additive,
    /// and it has no nullifier. Putting it in the same map would key it on
    /// something nullifier-shaped and let it shadow a spend record, which is
    /// the same hazard that decided the 44-byte length.
    pub fn insert_issuance(&mut self, hex_bytes: String, height: u64) {
        if !self.state.issuances.iter().any(|(h, _)| h == &hex_bytes) {
            self.state.issuances.push((hex_bytes, height));
        }
    }

    /// Every issuance seen, oldest first.
    pub fn issuances(&self) -> Vec<uv_kernel2::issuance::Issuance> {
        let mut seen: Vec<&(String, u64)> = self.state.issuances.iter().collect();
        seen.sort_by_key(|(_, h)| *h);
        seen.iter()
            .filter_map(|(hex_bytes, _)| {
                let raw = hex::decode(hex_bytes).ok()?;
                let arr: [u8; uv_kernel2::issuance::ISSUANCE_BYTES] =
                    raw.as_slice().try_into().ok()?;
                uv_kernel2::issuance::Issuance::decode(&arr)
            })
            .collect()
    }

    /// Every spend record confirmed in `from..=through`, in confirmation
    /// order (height, then position in the block).
    ///
    /// For [`crate::mirror`]: a mirror serves ranges wholesale rather than
    /// answering questions about particular nullifiers, because the question
    /// is the leak. Ordering is deterministic so two mirrors that saw the same
    /// chain produce byte-identical pages — which is what makes cross-checking
    /// a digest comparison instead of a diff.
    pub fn records_in(&self, from: u64, through: u64) -> Vec<(String, u64, u32, u32)> {
        let mut out: Vec<(String, u64, u32, u32)> = self
            .state
            .first
            .values()
            .filter(|e| e.height >= from && e.height <= through)
            .map(|e| (e.record_hex.clone(), e.height, e.index, e.vout))
            .collect();
        out.sort_by(|a, b| (a.1, a.2, a.3, &a.0).cmp(&(b.1, b.2, b.3, &b.0)));
        out
    }

    /// Every issuance record confirmed in `from..=through`, oldest first.
    pub fn issuances_in(&self, from: u64, through: u64) -> Vec<(String, u64)> {
        let mut out: Vec<(String, u64)> = self
            .state
            .issuances
            .iter()
            .filter(|(_, h)| *h >= from && *h <= through)
            .cloned()
            .collect();
...
mod tests {
    ...
    fn first_occurrence_wins_on_insert() {
        ...
        ix.insert(&rec(1, 10), 100, 0, 0);
        ix.insert(&rec(1, 99), 200, 3, 0); // a later, conflicting claim
...
mod rollback {
    ...
    fn entries_below_the_fork_survive() {
        ...
        ix.insert(&rec(1, 1), 100, 0, 0);
        ix.advance_to(100, "a".into());
        ix.insert(&rec(2, 2), 110, 0, 0);
