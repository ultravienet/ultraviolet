...
First, **the proof system is ours.** It was a general-purpose prover whose soundness
was one widely-reviewed artifact; it is now a circuit we wrote for this one job, at
~0.01–0.02 s and 5 MB per payment (`docs/benchmarks.html` has the case, the journal has
the decision). That trade bought four orders of magnitude and moved the soundness
burden onto us, which is the reason this brief exists. **A hand-written AIR is sound
only if every column is constrained.** A column nobody constrained is a free variable
for a prover, and a free variable in the wrong place mints money.
...
10. **Inflate the supply without publishing.** A coin whose genesis is not in a
    ...
    ([SPEC.md §9](SPEC.md#9-issuance-and-supply)). **This rule has already been wrong
...
- **The cryptography of the primitives.** The full list, because a scope boundary you
  cannot enumerate is not a boundary: **Poseidon2** (every money-path hash), **Keccak**
  (the trace Merkle commitments, FRI folding, and the Fiat–Shamir challenger —
  `air/src/prove.rs:44-57`; see `SPEC.md` §5.3), **FRI** soundness, and **ML-KEM-768 +
  X25519 + ChaCha20-Poly1305** off the money path. All assumed; we are not asking for a
  hash review.

  Two caveats on that. First, if you think our *parameters* are wrong — FRI at blowup 16
  with 25 queries for ~100-bit soundness — say so; that is in scope and we know those two
  constants are currently unpinned by any test. Second, Keccak was absent from this list
  until 2026-07-30, having been load-bearing in every proof for months while appearing in
  no specification and no assumption table. It was found by someone asking whether it was
  legacy code. We mention it because *how* it was missing is more useful to you than the
  fact that it was: nothing in our apparatus asks "which primitives are we trusting?", and
  the answer had to be recovered by reading `prove.rs`.

  (Note the boundary moved once before: the zero-knowledge *property of our composition*
  used to sit in this section as optional. It is attack #7 now, because it stopped being
  optional.)
...
- **Bitcoin.** Reorg handling and confirmation policy are modelled (`formal/reorg.qnt`)
  and tested against a real node (`btc/tests/reorgs_on_a_real_node.rs`), but Bitcoin's own consensus is
  assumed.
...
- **Negative tests per constraint family**, each tampering one thing and confirming
  ...
  **Read `air/COVERAGE.md` for how far that list is backed.** We mutation-tested it —
  delete each constraint region, see whether any test fails. **Latest sweep: 16 mutants,
  16 killed, 0 survive** (2026-07-30). Nine of those were survivors until that day,
  resting on an **inheritance** argument — byte-identical to constraints of the deleted
  signature circuit that a structural sweep had isolated. That argument was honest and it
  was not a test; it is retired. What closed them evaluates the constraint system
  concretely and records assertions **by index**, so a perturbation's failure set names
  which rules objected rather than reporting one boolean. The permutation's assertions are
  a measured prefix, which is what makes "our own rule fired, not the permutation covering
  for it" checkable.

  The same probe answers the question this brief opens with — *is every column
  constrained?* — for the first time mechanically: **all 361 of 361 columns provoke an
  objection when perturbed**, so there is no free variable in the circuit.

  **Where our evidence is still thin, and it is now a sharper place to look.** The probe
  changes **one cell at a time**: a hole needing two coordinated changes is beyond it, and
  that is precisely the shape of an under-constraint bug a careful attacker builds. And
  nothing here says the sixteen constraints are *sufficient* — only that each is
  load-bearing against tests we wrote. **Every cheap mechanical check is now spent. What
  is left is the part only a human finds, which is why you are reading this.**
...
- **Eight formal models** (`formal/`, Quint + Apalache), 73 checks in CI on every push:
  supply conservation, both strict-rule supply claims, the authorization rule, both
  reorg reconciliation claims, and off-circuit ancestry linkage all proven inductively at all
  depths — **seven inductive rows, each paired with a row asserting the induction fails on the
  variant that drops its rule**, so a reviewer can see the strengthenings carry the protocol's
  rules rather than the models' bookkeeping. Linkage is the one worth a reviewer's attention: its
  induction is stated through a **ghost variable** with one well-foundedness step discharged by
  hand in `formal/README.md`, and that hand step is exactly the sort of thing an audit should
  push on. Plus issuance with both historical
  free-mint counterexamples reproducing, reorgs,
...
- **Repository:** https://github.com/ultravienet/ultraviolet — one commit, always
  amended.
- **Build and test:** `cargo test --workspace` (237 tests, real proofs included);
  `cargo test -p uv-app --test the_whole_system_still_pays` for the end-to-end flow; `cargo run --release -p uv-air --bin
