...
final class Model: ObservableObject {
    ...
    struct Status {
        ...
        let stuck: Int
        /// What chain this view is, in words, and whether that is Bitcoin.
        /// The warning banner is driven by these rather than hardcoded, so it
        /// stops claiming "not connected" the moment it is connected.
        let backend: String
        let isBitcoin: Bool
    }
    ...
    func refreshStatus() {
        ...
        run({ try UV.callObject(["cmd": "status", "wallet": w]) }) { ok in
            ...
            self.status = Status(
                ...
                backend: ok["backend"] as? String ?? "unknown",
                isBitcoin: (ok["is_bitcoin"] as? NSNumber)?.boolValue ?? false
            )
        }
    }
    ...
    /// Run the on-device self-test and surface its transcript.
    func runDiagnostics() {
        busy = true
        Task.detached(priority: .userInitiated) {
            UVSelfTest.run()
            let text = (try? String(contentsOf: UVSelfTest.transcriptPath, encoding: .utf8)) ?? "no transcript"
            await MainActor.run {
